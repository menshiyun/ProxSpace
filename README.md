...
## Installation
 1. There are ~~two~~ one methods of installing the ProxSpace environment.
	* Downloading [the latest master](https://github.com/menshiyun/ProxSpace/archive/master.zip). This will upgrade the msys2 core packages and then will download and install every package required for compiling the Proxmark client and firmware. This can take some time.
 ...
 3. Install MSYS2 in the `ProxSpace` folder and name it `msys2` (e.g., `ProxSpace\msys2`).
	* Downloading [MSYS2](https://www.msys2.org/).
 4. Run `runme64.bat` depending on your Windows architecture.
 5. Get the Proxmark III repository you wish to compile. This can be done with git. For example `git clone https://github.com/RfidResearchGroup/proxmark3.git`.
 6. Go into the root directory of the repository you wish to compile. For example `cd proxmark3`.
 7. To build the project type `make clean && make all`. 
 8. In most cases the Proxmark III needs to be flashed with the just compiled firmware for details see **Firmware upgrading the Proxmark III**.
 9. To run the Proxmark III client type `./pm3`.
 10. Check your firmware revision on the Proxmark III with `hw ver`
 11. For basic help type `help`. Or for help on a set of sub commands type the command followed by help. For example `hf mf help`.
